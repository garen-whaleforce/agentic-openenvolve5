...
/**
 * BFF Proxy: GET /api/bff/earnings/range
 * 代理到後端 /api/earnings-calendar/range
 */
export async function GET(request: NextRequest) {
  ...
  if (!startDate || !endDate) {
    return NextResponse.json(
      { error: '缺少必要參數', message: 'start_date 和 end_date 為必填' },
      { status: 400 }
    );
  }
  ...
  try {
    const url = new URL(`${analysisApiBase}/api/earnings-calendar/range`);
    ...
    if (!response.ok) {
      ...
      console.error(`[BFF] earnings/range error: ${response.status}`, errorText);
      return NextResponse.json(
    ...
    console.error('[BFF] earnings/range exception:', error);
