#!/usr/bin/env python3
"""
2024 Validation Script - 50 samples with lookahead fix v2

Runs 50 samples from 2024 (balanced across Q1-Q4) using the fixed code.
Compares with previous results to verify fix effectiveness.

Usage:
    python run_validation_2024_50.py
"""
...
os.environ["CALL_CACHE_VERSION"] = "v2.1"  # Bump version to invalidate old cache
...
def get_2024_balanced_samples(limit: int = 50) -> List[Dict]:
...
class ProgressTracker:
    ...
    def add_result(self, result: Dict):
        ...
        now = time.time()
        if now - self.last_report_time >= 30 or self.completed % 5 == 0:
...
async def run_with_semaphore(
    ...
    async with semaphore:
        ...
        print(f"[{idx+1}/{tracker.total}] {symbol} {quarter}: {status} | {pred} | Correct={correct_str} | {trade}", flush=True)


async def main():
    print("=" * 70)
    print("2024 VALIDATION - 50 SAMPLES (Lookahead Fix v2)")
    print("=" * 70)
    ...
    output_file = f"validation_2024_50_{timestamp}.csv"
